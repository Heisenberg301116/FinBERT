...
# Labels
labels = load_pickle(path + '/data/evaluation_labels/labels.pickle')
...
class BERT_MODEL():
    ...
    def __init__(self, bert_model_name):
        ...
        """
        self.bert_model_name = bert_model_name      # bert-qa by default
    ...
    def get_model(self):
        ...
        if self.bert_model_name == "bert-base":
            ...
            get_model("finbert-domain")     # get_models() here is of utils.py file. If we had used self.get_model() then the get_model() of this class would have been called !!!!
            ...
            get_model("bert-qa")
            model_path = Path.cwd()/'model/bert-qa'         # 'bert-qa' : pre-trained BERT model fine-tuned on the MS Macro passage
...
class PointwiseBERT():
    def __init__(self, config, tokenizer, model, optimizer):
        self.config = config
        self.train_set = load_pickle(self.config['train_set'])          # /data/train_set.pickle
        # Load validation set
    ...
    def get_input_data(self, dataset):
        ...
        for i, seq in enumerate(tqdm(dataset)):
            ...
            for docid in cands:
                # Map the docid to text but first confirm whether the candidate answer's docid actually exists in our database or not 
    ...
    def train_pointwise(self):          # fine tuning BERT-QA on FiQA dataset. BERT QA was already downloaded model which is further pre-trained model on MS-MACRO dataset
        ...
        for epoch in range(n_epochs+1):
            if epoch > 0:
                # Saving fine tuned BERT-QA model (now called FinBERT-QA model):
                pwd_path = os.getcwd()
                ...
                save_loc = fin_dir + str(epoch)+ '_pointwise_finbert-qa.pt'
                torch.save(self.model.state_dict(), save_loc)
                
                print("==================================================================================================================================================================================================================\n")
                print("\nEpoch {}:".format(epoch))
                print("\t Train Loss: {} | Train Accuracy: {}%".format(round(train_loss, 3), round(train_acc*100, 2)))
                print("\t Validation Loss: {} | Validation Accuracy: {}%".format(round(valid_loss, 3), round(valid_acc*100, 2)))
                print("\t Saved location of model = ",save_loc)
                print("\n==================================================================================================================================================================================================================")
            ...
            train_loss, train_acc = self.train(self.model, train_dataloader, self.optimizer, scheduler)

            # Evaluate validation loss
            valid_loss, valid_acc = self.validate(self.model, validation_dataloader)
...
class FinBERT_QA():
    ...
    def __init__(self, config, state):
        ...
        self.bert_model_name = self.config['bert_model_name']       # bert-qa by default
        self.device = torch.device('cuda' if config['device'] == 'gpu' else 'cpu')
        self.max_seq_len = self.config['max_seq_len']
        
        # Assign lucene id
        if(state == 'TRAINING' or state == 'PREDICTION'):
            self.doc_index_id = self.config['doc_index_id']

        # Load the BERT tokenizer.
        print('\nLoading BERT tokenizer...')
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
        
        # Initialize model for training/fine tuning
        if state == 'TRAINING':
            print("\nLoading pre-trained BERT model...")
            self.model = BERT_MODEL(self.bert_model_name).get_model().to(self.device)       # => self.model = 'bert-qa' : pre-trained BERT model fine-tuned on the MS Macro passage

        else:
            print("\nLoading pre-trained BERT model...")
            self.model = BERT_MODEL(self.bert_model_name).get_model().to(self.device)       # => self.model = 'bert-qa' : pre-trained BERT model fine-tuned on the MS Macro passage
            
            # Setting fine tuned model location
            ...
            if torch.cuda.is_available():               # Using CUDA compatible torch gpu version
                # If GPU is available, load the model on the GPU
                self.model.load_state_dict(torch.load(model_path), strict=False)
            else:                                       # Using torch cpu version
                # If GPU is not available, load the model on the CPU
    ...
    def run_train(self):
        ...
        if learning_approach == 'pointwise':
            trainer = PointwiseBERT(self.config, self.tokenizer, self.model, optimizer)             # trainer is an object of PointwiseBERT class
            trainer.train_pointwise()        # fine tuning BERT-QA on FiQA dataset. BERT QA was already downloaded model which is further pre-trained model on MS-MACRO dataset
        else:
    ...
    def evaluate_model(self):
        ...
        """
        # Load test set
        #print("=================> Loading but os.getcw() = ", os.getcwd())
        ...
        if self.config['use_trained_model'] == True:            # by default, use_trained_model is False
            # Load fine tuned model path
            model_name = get_trained_model(bert_finetuned_model)
            model_path = path + "/model/trained/" + bert_finetuned_model + "/" + model_name
        else:
    ...
    def search(self, query, top_k, query_id):
        """Search engine. Retrieves and re-ranks the answer candidates given a query.
        Renders the top-k answers for a query.
        """

        # first retrieve 'k' relevant answers using BM-25 and then retrieve top_k answers using FinBERT re-ranker.
        hits = Search_Top_k_Docs(self.doc_index_id, query_id, query, 50)         # hits will store top 50 answers
        if(hits == -1):         # => error
            print("Error in finding top 50 hits !!!")
            sys.exit()

        cands = []
        for docid in hits:
            cands.append(int(docid))
        ...
        if len(cands) == 0:
            ...
            print("\n==================================================================================================================================================================================================================")
            print("Top-{} Answers: \n".format(top_k))
            for i in range(0, top_k):              # top_k represents the top k answers to be selected and displayed to user by our fine tuned model from the top 50 selected answers
                print("{}.\t{}\n".format(i+1, docid_to_text[self.rank[i]]))
                output += docid_to_text[self.rank[i]] + '\n\n'
            print("\n==================================================================================================================================================================================================================")
    
            # Saving 'output' into json file:
            file_path = "data/predictions/passages.json"
